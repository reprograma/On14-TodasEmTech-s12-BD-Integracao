...
const createArtists = async (req, res) => {
  try {
    ...
    res.status(201).json({
      massage: `Artista: ${artist.artista}, criade com sucesso!`,
      artistSaved,
    });
  } catch (error) {
    res.status(500).json({
...
const updateArtists = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: `Artista: ${artistFound.artista}, atualizade com sucesso!`,
      artistSaved,
    });
  } catch (error) {
    res.status(500).json({
...
const deleteArtists = async (req, res) => {
  try {
    ...
    if (artistFound) {
      artistFound.delete();
      res.status(200).json({
        message: `Artista: ${artistFound.artista}, excluíde com sucesso!`,
      });
    }
  } catch (error) {
    res.status(500).json({
